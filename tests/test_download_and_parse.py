...
from scripts.download_and_parse_data import parse_conllu
...
def test_parse_conllu():
    sentences = parse_conllu(test_conllu)

    assert len(sentences) == 2

    sentence_0 = sentences[0]
    words_0, tags_0 = zip(*sentence_0)
    assert " ".join(words_0) == "From the AP comes this story :"
    assert " ".join(tags_0) == "ADP DET PROPN VERB DET NOUN PUNCT"


def test_correct_parsing_and_saving():
    response = requests.get(
        "https://raw.githubusercontent.com/UniversalDependencies/UD_English-EWT/master/en_ewt-ud-train.conllu"
    )
    sentences = parse_conllu(response.text)
    second_sentence = sentences[1]
    words, _ = zip(*second_sentence)
    assert (
        " ".join(words)
